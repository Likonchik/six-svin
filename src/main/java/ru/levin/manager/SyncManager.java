...
public class SyncManager implements IMinecraft {
    ...
    private volatile List<AbstractClientPlayer> cachedPlayers = Collections.emptyList();
    ...
    public float getBodyYaw() {
        ...
        if ((dx * dx + dz * dz) > 0.0025f) {
            offset = (float) (Mth.atan2(dz, dx) * (180f / Math.PI) - 90.0f);
        }
        ...
        float deltaBodyYaw = Mth.clamp(Mth.wrapDegrees(lastYaw - (visualBodyYaw + Mth.wrapDegrees(offset - visualBodyYaw) * 0.3f)), -45.0f, 75.0f);
    ...
    public void onEvent(Event event) {
        ...
        switch (event) {
            case EventUpdate ignored -> {
                ...
                cachedEntities = ImmutableList.copyOf(mc.level.entitiesForRendering());
                cachedPlayers = ImmutableList.copyOf(mc.level.players());
                cachedItems = ImmutableList.copyOf(mc.player.getArmorSlots());
            }
            ...
            case EventPacket ep -> {
                if (ep.getPacket() instanceof ClientboundSetTimePacket) {
                    ...
                    currentTPS = Mth.clamp(currentTPS, 0F, 20F);
