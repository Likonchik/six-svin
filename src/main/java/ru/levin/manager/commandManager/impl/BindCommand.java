...
public class BindCommand extends Command {
    ...
    private CompletableFuture<Suggestions> suggestModules(CommandContext<SharedSuggestionProvider> context, SuggestionsBuilder builder) {
        Manager.FUNCTION_MANAGER.getFunctions().forEach(f -> builder.suggest(f.name));
        return builder.buildFuture();
    }

    private CompletableFuture<Suggestions> suggestKeys(CommandContext<SharedSuggestionProvider> context, SuggestionsBuilder builder) {
        KeyMappings.getAllKeys().forEach(builder::suggest);
        return builder.buildFuture();
    }

    private void listBoundKeys() {
        ClientManager.message(ChatFormatting.GRAY + "Список всех модулей с клавишами:");
        for (Function f : Manager.FUNCTION_MANAGER.getFunctions()) {
            if (f.bind == 0) continue;
            String keyName = KeyMappings.keyMappings(f.bind);
            ClientManager.message(f.name + " [" + ChatFormatting.GRAY + keyName + ChatFormatting.RESET + "]");
        }
    }
    ...
    private void clearAllBindings() {
        ...
        ClientManager.message(ChatFormatting.GREEN + "Все клавиши были отвязаны (кроме ClickGUI)");
    }
    ...
    private void addKeyBinding(String moduleName, String keyName) {
        ...
        module.bind = key;
        ClientManager.message("Клавиша " + ChatFormatting.GRAY + keyName + ChatFormatting.WHITE + " была привязана к модулю " + ChatFormatting.GRAY + module.name);
    }
    ...
    private void removeKeyBinding(String moduleName, String keyName) {
        ...
        module.bind = 0;
        ClientManager.message("Клавиша " + ChatFormatting.GRAY + keyName + ChatFormatting.WHITE + " была отвязана от модуля " + ChatFormatting.GRAY + module.name);
    }
}
