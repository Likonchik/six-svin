...
public class UnHookCommand extends Command {
    ...
    public static final File CUSTOM_PATH_FILE = new File(mc.gameDirectory, "files\\modules\\UnHook.ew");
    ...
    private CompletableFuture<Suggestions> suggestFolders(CommandContext<SharedSuggestionProvider> context, SuggestionsBuilder builder) {
    ...
    private void savePath(String path) {
        try {
            ...
            try (FileWriter writer = new FileWriter(CUSTOM_PATH_FILE, false)) {
                ...
                ClientManager.message(ChatFormatting.GREEN + "Путь сохранён: " + ChatFormatting.WHITE + path);
            }

        } catch (IOException e) {
            ClientManager.message(ChatFormatting.RED + "Ошибка при сохранении пути: " + e.getMessage());
        }
    }
}
