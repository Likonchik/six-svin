...
public class Dragging {
    ...
    public void onDraw(double mouseX, double mouseY, Window window) {
        ...
        if (borderAlpha > 0f) {
            ...
            RenderUtil.drawRoundedBorder(new PoseStack(), drawX, drawY, drawWidth, drawHeight, 3f, 0.01f, alphaColor);
        }

        float targetTipAlpha = dragging ? 1f : 0f;
        tipAlpha += (targetTipAlpha - tipAlpha) * 0.1f;
        tipAlpha = Mth.clamp(tipAlpha, 0f, 1f);
        if (tipAlpha > 0f) {
    ...
    public void renderGuides(Window window) {
        ...
        guideAlpha = Mth.clamp(guideAlpha, 0f, 1f);
