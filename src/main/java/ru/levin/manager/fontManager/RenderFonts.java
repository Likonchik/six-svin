...
public class RenderFonts implements Closeable, IMinecraft {
    ...
    private final Map<ResourceLocation, ObjectList<DrawEntry>> GLYPH_PAGE_CACHE = new ConcurrentHashMap<>();
    ...
    public void drawLeftAligned(PoseStack ms, String text, float x, float y, int color) {
        render(ms, text, x, y, color, false);
    }

    public void drawRightAligned(PoseStack ms, String text, float x, float y, int color) {
        float width = getWidth(text);
        render(ms, text, x - width, y, color, false);
    }

    public void centeredDraw(PoseStack ms, String text, float x, float y, int color) {
        float width = getWidth(text);
    ...
    public void drawClipped(PoseStack ms, String text, float maxWidth, float x, float y, int color) {
    ...
    private void render(PoseStack ms, String text, float x, float y, int color, boolean isGradient) {
        if (text == null || text.isEmpty()) return;
        ...
        Map<ResourceLocation, List<DrawEntry>> localCache = new Object2ObjectOpenHashMap<>();
        ...
        localCache.forEach((identifier, entries) -> {
            ...
            for (DrawEntry entry : entries) {
                ...
                bufferBuilder.addVertex(matrix4f, entry.atX, entry.atY + h, 0).setUv(u1, v2).setColor(entry.r, entry.g, entry.b, a);
                bufferBuilder.addVertex(matrix4f, entry.atX + w, entry.atY + h, 0).setUv(u2, v2).setColor(entry.r, entry.g, entry.b, a);
                bufferBuilder.addVertex(matrix4f, entry.atX + w, entry.atY, 0).setUv(u2, v1).setColor(entry.r, entry.g, entry.b, a);
                bufferBuilder.addVertex(matrix4f, entry.atX, entry.atY, 0).setUv(u1, v1).setColor(entry.r, entry.g, entry.b, a);
            }
    ...
    public void renderGradientText(PoseStack matrixStack, String text, float x, float y, int color1, int color2) {
        renderGradientInternal(matrixStack, text, x, y, color1, color2, 0f, false);
    }

    public void renderAnimatedGradientText(PoseStack matrixStack, String text, float x, float y, int color1, int color2, float time) {
        renderGradientInternal(matrixStack, text, x, y, color1, color2, time, true);
    }

    private void renderGradientInternal(PoseStack matrixStack, String text, float x, float y,
        ...
        Map<ResourceLocation, List<GradientDrawEntry>> textureMap = new Object2ObjectOpenHashMap<>();
        ...
        textureMap.forEach((texture, entries) -> {
            ...
            for (GradientDrawEntry entry : entries) {
                ...
                bufferBuilder.addVertex(matrix4f, entry.x, h, 0).setUv(u1, v2).setColor(entry.r, entry.g, entry.b, entry.a);
                bufferBuilder.addVertex(matrix4f, entry.x + w, h, 0).setUv(u2, v2).setColor(entry.r, entry.g, entry.b, entry.a);
                bufferBuilder.addVertex(matrix4f, entry.x + w, 0, 0).setUv(u2, v1).setColor(entry.r, entry.g, entry.b, entry.a);
                bufferBuilder.addVertex(matrix4f, entry.x, 0, 0).setUv(u1, v1).setColor(entry.r, entry.g, entry.b, entry.a);
            }
            RenderUtil.render3D.endBuilding(bufferBuilder);
        });
    ...
    private Glyph createGlyph(char glyphChar) {
        ...
        GlyphMap newMap = new GlyphMap((char) base, (char) (base + charsPerPage), font,
                ResourceLocation.fromNamespaceAndPath("font", "temp/" + id), padding, sharedFontRenderContext);
        maps.add(newMap);
    ...
    private class GlyphMap {
        ...
        private final ResourceLocation bindToTexture;
        ...
        GlyphMap(char from, char to, Font font, ResourceLocation id, int padding, FontRenderContext frc) {
            this.fromIncl = from;
            this.toExcl = to;
            this.font = font;
        ...
        void destroy() {
            mc.getTextureManager().release(bindToTexture);
            glyphs.clear();
            width = -1;
            height = -1;
            generated = false;
        ...
        private void registerBufferedImageTexture(ResourceLocation id, BufferedImage img) {
            try (NativeImage nativeImage = new NativeImage(NativeImage.Format.RGBA, img.getWidth(), img.getHeight(), false)) {
                ...
                for (int i = 0; i < pixels.length; i++) {
                    ...
                    nativeImage.setPixelRGBA(i % img.getWidth(), i / img.getWidth(), abgr);
                }
