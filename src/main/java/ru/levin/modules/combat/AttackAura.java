...
public class AttackAura extends Function {
    ...
    public void onEvent(Event event) {
        ...
        if (event instanceof EventUpdate) {
            ...
            if (target == null) {
                Manager.ROTATION.set(player.getYRot(), player.getXRot());
    ...
    protected void onDisable() {
        if (target != null && isValidTarget(target)) {
            ...
            if (modeName.equals("FunTime") || modeName.equals("HollyWorld") || modeName.equals("ReallyWorld")) {
                ...
                Manager.ROTATION.set(mc.player.getYRot(), mc.player.getXRot());
            }
        }
    ...
    private boolean isValidTarget(LivingEntity entity) {
        if (entity == null || entity.isDeadOrDying() || !entity.isAlive() || entity == mc.player) return false;
        ...
        } else if (entity instanceof Villager && !targets.get("Жители")) return false;
        else if (entity instanceof Mob || entity instanceof Animal) {
    ...
    private void handleAttackAndRotation(LivingEntity t) {
        ...
        if (mode.is("SpookyTime")) {
            ...
            if (target == null) {
                randomYawOffset = 0;
                randomPitchOffset = 0;
                float centerYaw = mc.player.getYRot();
                float centerPitch = mc.player.getXRot();

                {
                    prevBodyYaw = bodyYaw;
                    prevBodyPitch = bodyPitch;

                    float yawDiff = Mth.wrapDegrees(centerYaw - bodyYaw);
                    float pitchDiff = centerPitch - bodyPitch;

                    float yawStep = Mth.clamp(yawDiff, -45, 45);
                    float pitchStep = Mth.clamp(pitchDiff, -45, 45);

                    bodyYaw += yawStep;
                    bodyPitch = Mth.clamp(bodyPitch + pitchStep, -90f, 90f);
                }

                {
                    ...
                    float yawDiff = Mth.wrapDegrees(centerYaw - headYaw);
                    float pitchDiff = centerPitch - headPitch;

                    float yawStep = Mth.clamp(yawDiff, -50, 50);
                    float pitchStep = Mth.clamp(pitchDiff, -50, 50);

                    headYaw += yawStep;
                    headPitch = Mth.clamp(headPitch + pitchStep, -90f, 90f);
                }
                return;
            }
            ...
            Vec3 targetPos;
            {
                ...
                switch (randPoint) {
                    case 0 -> y += target.getBbHeight() * 0.9;
                    case 1 -> y += target.getBbHeight() * 0.75;
                    case 2 -> y += target.getBbHeight() * 0.5;
                    case 3 -> y += target.getBbHeight() * 0.25;
                }
            ...
            Vec2 rot;
            {
                ...
                rot = new Vec2((float) pitch, (float) yaw);
            }
            {
                prevBodyYaw = bodyYaw;
                prevBodyPitch = bodyPitch;

                float yawDiff = Mth.wrapDegrees(rot.y - bodyYaw);
                float pitchDiff = rot.x - bodyPitch;

                bodyYaw += Mth.clamp(yawDiff, -45, 45);
                bodyPitch = Mth.clamp(bodyPitch + pitchDiff, -45, 45);
            }
            ...
            Vec2 headRotation;
            {
                ...
                float yawDiff = Mth.wrapDegrees(rot.y - headYaw);
                float pitchDiff = rot.x - headPitch;

                headYaw += Mth.clamp(yawDiff, -50, 50);
                headPitch = Mth.clamp(pitchDiff, -50, 50);

                headRotation = new Vec2(
        ...
        if (mode.is("FunTime")) {
           ...
           if (System.currentTimeMillis() - lastHitMs < 450) {
               ...
               float finalYaw = mc.player.getYRot() + yawOffset + yawOffset2;
    ...
    private void koopinVector(LivingEntity entity, boolean attackContext) {
        Vec3 head = entity.getEyePosition().add(0, entity.getBbHeight(), 0);
        Vec3 chest = entity.getEyePosition().add(0, entity.getEyeHeight() / 2.0f, 0);
        Vec3 legs = entity.getEyePosition().add(0, 0.05, 0);
        Vec3[] points = new Vec3[]{head, chest, legs};
        ...
        float yawDelta = Mth.wrapDegrees(yawTo - currYaw);
        ...
        float np = Mth.clamp(currPitch + (pitchDelta > 0 ? addPitch : -addPitch), -90.0f, 90.0f);
    ...
    private void funtime(LivingEntity entity) {
        ...
        Vec3 targetPos = new Vec3(base.x, base.y + entity.getBbHeight() * mul, base.z);
        ...
        if (now - lastSwitch > 200 + random.nextInt(250)) {
            ...
            float distanceFactor = (float) Mth.clamp(dist / 6.0f, 0.4f, 1.0f);
        ...
        float totalYawOffset = (float) Mth.clamp(jitterYaw + sway, -halfWidth * 8.5f, halfWidth * 8.5f);
    ...
    private void hollyworld(LivingEntity entity, boolean force) {
        ...
        Vec3 aim = new Vec3(base.x, base.y + entity.getBbHeight() * mul, base.z);
        ...
        if (force) {
            Manager.ROTATION.set(yaw, Mth.clamp(pitch, -89.9f, 89.9f));
        } else {
    ...
    public void attackTarget(Player player) {
        ...
        boolean canStartSprint = mc.player.input.forwardImpulse > 0
    ...
    private boolean canAttack() {
        if (noAttackIfEat.get() && mc.player.isUsingItem() && !mc.player.getUseItem().is(Items.SHIELD)) return false;

        if (System.currentTimeMillis() < cpsLimit
                || (!(mc.player.getMainHandItem().is(Items.MACE))
                && mc.player.getAttackStrengthScale(mc.getTimer().getGameTimeDeltaPartialTick(true)) < 0.9F)) return false;

        boolean restrict = mc.player.hasEffect(MobEffects.BLINDNESS)
                ...
                || (mc.player.isInLiquid() && !mc.options.keyJump.isDown())
                || MoveUtil.isInWeb();
        ...
        if (setting.get("Только критами") && !restrict) {
            return needSpace || (!mc.player.onGround() && mc.player.fallDistance > 0.0f);
        }
        return true;
    }
    ...
    private boolean shieldBreaker(boolean instant) {
        ...
        if (((Player) target).getOffhandItem().getItem() != Items.SHIELD
                && ((Player) target).getMainHandItem().getItem() != Items.SHIELD) return false;

        if (axeSlot >= 9) {
            mc.gameMode.handleInventoryMouseClick(mc.player.containerMenu.containerId, axeSlot, mc.player.getInventory().selected, ClickType.SWAP, mc.player);
    ...
    private Vec3 predictPos(LivingEntity e) {
        Vec3 p = e.position();
