        ...
        if (event instanceof EventUpdate) {
            ...
            if (lastNick != null && lastBet >= 0) {
                if (lastBet >= fromMoney.get().intValue() && lastBet <= beforeMoney.get().intValue()) {
                    mc.player.connection.sendChat("/duel accept " + lastNick);
                }
                lastNick = null;
                lastBet = -1;
            }
            ...
            if (eventPacket.getPacket() instanceof ClientboundSystemChatPacket) {
                Component message = ((ClientboundSystemChatPacket) eventPacket.getPacket()).content();
