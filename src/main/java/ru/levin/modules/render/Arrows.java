...
public class Arrows extends Function {
    ...
    private void drawArrow(PoseStack stack, Player target, double x, double z, int baseColor) {
        var player = mc.player;
        ...
        float desiredAngle = Mth.wrapDegrees(getRotationTo(new Vector2f((float) x, (float) z)) - player.getYRot());
    ...
    private float smoothAngle(UUID id, float targetAngle) {
        ...
        float delta = Mth.wrapDegrees(targetAngle - prev);
        float factor = 0.08f;
