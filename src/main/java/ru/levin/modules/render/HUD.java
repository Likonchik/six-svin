...
public class HUD extends Function {
    public final MultiSetting setting = new MultiSetting(
            "Элементы",
            Arrays.asList("WaterMark", "TargetHUD", "KeyBinds", "StaffList", "PotionHUD", "ItemCoolDownHUD", "Coordinates / TPS","ArmorHUD", "Notifications", "MediaPlayer"),
            new String[]{"WaterMark", "TargetHUD", "KeyBinds", "StaffList", "PotionHUD", "ItemCoolDownHUD", "Coordinates / TPS","ArmorHUD", "Notifications", "MediaPlayer"}).withDesc("Отображаемые элементы худа");


    private final ModeSetting hudColor = new ModeSetting("Цвет худа","Обычный","Обычный","Зависит от темы").withDesc("Цвет элементов худа");
    private final ModeSetting gradientType = new ModeSetting(() -> hudColor.is("Зависит от темы"),"Тип градиента", "Слева направо", "Слева направо", "Справа налево").withDesc("Направление градиента");

    private final SliderSetting customAlpha = new SliderSetting("Прозрачность", 120, 120, 255, 5).withDesc("Прозрачность фона худа");
    ...
    private final SliderSetting roundingSilaSanya = new SliderSetting("Закругление головы", 2f, 0f, 12f, 1f).withDesc("Закругление головы в TargetHUD");
    ...
    private void armor(EventRender2D eventRender2D) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (!itemStack.isEmpty()) {
                eventRender2D.getDrawContext().pose().pushPose();
                eventRender2D.getDrawContext().pose().translate(startX, y + 0.2f, 0);
                eventRender2D.getDrawContext().pose().scale(1, 1, 1);
                eventRender2D.getDrawContext().renderItem(itemStack, 0, 0, 0);
                eventRender2D.getDrawContext().renderItemDecorations(mc.font, itemStack, 0, 0);
                eventRender2D.getDrawContext().pose().popPose();
                startX -= 20;
            }
        }
    }
    ...
    private void updateStaffPlayers(Minecraft mc) {
        staffPlayers.clear();
        addedPlayers.clear();

        Map<String, PlayerInfo> nameToEntry = new HashMap<>(mc.player.connection.getOnlinePlayers().size() + 4);
        ...
        for (PlayerTeam team : scoreboard.getPlayerTeams()) {
            Component prefixComponent = team.getPlayerPrefix();
            ...
            for (String member : team.getPlayers()) {
                ...
                PlayerInfo entry = nameToEntry.get(member.toLowerCase(Locale.ROOT));
    ...
    private void potion(EventRender2D eventRender2D) {
        ...
        List<MobEffectInstance> activeEffects = new ArrayList<>(mc.player.getActiveEffects());
        float maxWidth = 100;
        ...
        for (MobEffectInstance eff : activeEffects) {
            String name = I18n.get(eff.getEffect().value().getDescriptionId());
        ...
        if (alpha <= 240) {
            if (blur.get()) {
                drawBlur(eventRender2D.getDrawContext().pose(), posX, posY + headerHeight - 1, maxWidth, potionListHeightDynamic + 6, new Vector4f(0, 3, 3, 0), 12, Color.white.getRGB());
            }
        }
        ...
        if (hudColor.is("Обычный")) {
            drawRoundedRect(eventRender2D.getDrawContext().pose(), posX, posY, maxWidth, headerHeight + 1, new Vector4f(3, 0, 0, 3), hud_color);
        } else {
            ...
            rectRGB(eventRender2D.getDrawContext().pose(), posX, posY, maxWidth, headerHeight + 1, corner, c1, c2, c3, c4);
        }

        RenderUtil.drawTexture(eventRender2D.getDrawContext().pose(), "images/hud/potion.png", posX + maxWidth - 16, posY + 4.5f, 11, 11, 0, Color.white.getRGB());

        FontUtils.durman[15].drawLeftAligned(eventRender2D.getDrawContext().pose(), "Effects", posX + 10, posY + 5f, -1);

        drawRoundedRect(eventRender2D.getDrawContext().pose(), posX, posY + headerHeight - 1, maxWidth, potionListHeightDynamic + 6, new Vector4f(0, 3, 3, 0), new Color(22, 22, 22, alpha).getRGB());
        ...
        for (MobEffectInstance eff : activeEffects) {
            MobEffect effect = eff.getEffect().value();
            ...
            TextureAtlasSprite texture = spriteManager.get(holder);

            float finalYOffset = yOffset;
            list.add(() -> {eventRender2D.getDrawContext().blit((int) (posX + padding - 1.5f), (int) finalYOffset - 1, 0, 9, 9, texture);});

            RenderUtil.drawRoundedRect(eventRender2D.getDrawContext().pose(), posX + padding + 9, finalYOffset - 1f, 1.2f, 9, 0, new Color(255, 255, 255, 120).getRGB());

            FontUtils.durman[13].drawLeftAligned(eventRender2D.getDrawContext().pose(), displayName, posX + padding + 14f, yOffset - 1f, color);

            RenderUtil.drawRoundedRect(eventRender2D.getDrawContext().pose(), posX + maxWidth - maxDurationWidth - padding - 7, yOffset - 1, 8 + maxDurationWidth, 10, 1, hud_color);

            FontUtils.durman[13].drawLeftAligned(eventRender2D.getDrawContext().pose(), duration, posX + maxWidth - maxDurationWidth - padding - 3, yOffset - 0.3f, color);
    ...
    private String formatDuration(MobEffectInstance eff) {
        if (eff.isInfiniteDuration() || eff.getDuration() > 18000) {
            return "**:**";
        }
        String raw = MobEffectUtil.formatDuration(eff, 1.0F, 20.0f).getString();
    ...
    private void cooldown(EventRender2D eventRender2D) {
        ...
        for (Item item : TRACKED_ITEMS) {
            if (manager.isOnCooldown(item)) {
                activeItems.add(item);

                ItemStack stack = new ItemStack(item);
                String itemName = ITEM_NAMES.getOrDefault(item, stack.getHoverName().getString());

                Object rawEntry = accessor.getCooldowns().get(item);

                float remainingSeconds = 0f;
                if (rawEntry instanceof ItemCooldownEntryAccessor entry) {
                    int end = entry.getEndTime();
                    int current = accessor.getTickCount();
                    float remainingTicks = end - (current + mc.getTimer().getGameTimeDeltaPartialTick(true));
        ...
        if (alpha <= 240) {
            if (blur.get()) {
                drawBlur(eventRender2D.getDrawContext().pose(), posX, posY + headerHeight - 1, maxWidth, cooldownListHeightDynamic + 6, new Vector4f(0, 3, 3, 0), 12, Color.white.getRGB());
            }
        }
        ...
        RenderUtil.drawTexture(eventRender2D.getDrawContext().pose(), "images/hud/cooldown.png", posX + maxWidth - 17, posY + 4.5f, 11, 11, 0, Color.white.getRGB());

        FontUtils.durman[15].drawLeftAligned(eventRender2D.getDrawContext().pose(), "Cooldowns", posX + 10, posY + 5f, -1);

        drawRoundedRect(eventRender2D.getDrawContext().pose(), posX, posY + headerHeight - 1, maxWidth, cooldownListHeightDynamic + 6, new Vector4f(0, 3, 3, 0), new Color(22, 22, 22, alpha).getRGB());
        ...
        for (Item item : activeItems) {
            ...
            RenderUtil.drawRoundedRect(eventRender2D.getDrawContext().pose(), posX + padding + 10, yOffset - 0.5f, 1.2f, 9, 0, new Color(255, 255, 255, 120).getRGB());

            FontUtils.durman[13].drawLeftAligned(eventRender2D.getDrawContext().pose(), itemName, posX + padding + 14f, yOffset - 0.3f, -1);

            float timeWidth = FontUtils.durman[13].getWidth(timeLeft);
            RenderUtil.drawRoundedRect(eventRender2D.getDrawContext().pose(), posX + maxWidth - timeWidth - padding - 5, yOffset - 1, 6 + timeWidth, 10, 1, hud_color);

            FontUtils.durman[13].drawLeftAligned(eventRender2D.getDrawContext().pose(), timeLeft, posX + maxWidth - timeWidth - padding - 2, yOffset - 0.3f, -1);
    ...
    private void staffList(EventRender2D render2D) {
        ...
        if (alpha <= 240) {
            if (blur.get()) {
                drawBlur(render2D.getDrawContext().pose(), posX, posY + headerHeight - 1, widthDynamic, hDynam + 6, new Vector4f(0, 3, 3, 0), 12, Color.white.getRGB());
            }
        }
        StyleManager theme = Manager.STYLE_MANAGER;
        Color upColor = new Color(theme.getFirstColor());
        ...
        if (hudColor.is("Обычный")) {
            drawRoundedRect(render2D.getDrawContext().pose(), posX, posY, widthDynamic, headerHeight + 1, corner, hud_color);
        } else {
            int left   = ColorUtil.gradient(10,   90, upColor.getRGB(), downColor.getRGB());
            int right  = ColorUtil.gradient(10,    0, upColor.getRGB(), downColor.getRGB());
            int top    = ColorUtil.gradient(10,  180, upColor.getRGB(), downColor.getRGB());
            ...
            rectRGB(render2D.getDrawContext().pose(), posX, posY, widthDynamic, headerHeight + 1, corner, c1, c2, c3, c4);
        }


        RenderUtil.drawTexture(render2D.getDrawContext().pose(), "images/hud/staff.png", posX + widthDynamic - headerHeight, posY + 4, 12, 12, 0, Color.white.getRGB());

        fontBig.drawLeftAligned(render2D.getDrawContext().pose(), "StaffList",posX + 10, posY + 5f,-1);

        drawRoundedRect(render2D.getDrawContext().pose(), posX, posY + headerHeight - 1, widthDynamic, hDynam + 6, new Vector4f(0, 3, 3, 0), new Color(22, 22, 22, alpha).getRGB());
        ...
        if (!staffPlayers.isEmpty()) {
            ...
            for (StaffPlayer staff : staffPlayers) {
                ...
                PlayerInfo playerInfo = playerInfoMap.get(staffname);
                if (playerInfo != null) {
                    if (!(staff.getStatus() == StaffPlayer.Status.VANISHED || staff.getStatus() == StaffPlayer.Status.SPEC)) {
                        RenderAddon.drawStaffHead(render2D.getDrawContext().pose(), playerInfo.getSkin().texture(), posX + padding, baseY - 1f, 9, 3);
                    }
                } else {
                    RenderUtil.drawTexture(render2D.getDrawContext().pose(), "images/hud/staffvanish.png", posX + padding, baseY - 1f, 9, 9, 3, Color.white.getRGB());
                }

                RenderUtil.drawRoundedRect(render2D.getDrawContext().pose(), posX + padding + 11, baseY - 0.8f, 1.2f, 9, 0, new Color(255, 255, 255, 120).getRGB());

                fontSmall.drawLeftAligned(render2D.getDrawContext().pose(), staffname, posX + padding + 16f, baseY - 0.5f, Color.WHITE.getRGB());

                fontSmall.drawLeftAligned(render2D.getDrawContext().pose(), status, posX + widthDynamic - statusWidth - 4, baseY - 1, getStatusColor(staff.getStatus()));
    ...
    private void targethud(EventRender2D render2D) {
        ...
        float rawHealth = Mth.clamp(ServerUtil.getHealth(target), 0.0F, 1.0F);
        float rawAbsorption = Mth.clamp(target.getAbsorptionAmount(), 0.0F, 7.1F);
        ...
        render2D.getMatrixStack().pushPose();
        RenderAddon.sizeAnimation(render2D.getMatrixStack(), x + 60.0F, y + 17.5f, scale);
        drawRoundedRect(render2D.getDrawContext().pose(), x, y, 120, 35, 3, hud_color);
        ...
        FontUtils.durman[15].drawLeftAligned(render2D.getDrawContext().pose(), displayName, x + 35, y + 5f, -1);

        RenderAddon.drawHead(render2D.getDrawContext().pose(), target, x + 4, y + 3.5f, 28, roundingSilaSanya.get().floatValue());

        drawRoundedRect(render2D.getDrawContext().pose(), x + 34.2F, y + 26F, 82, 5.0F, 0, new Color(44, 41, 42, 255).getRGB());
        ...
        rectRGB(render2D.getDrawContext().pose(), x + 34.2F, y + 26F, 82 * lastHealth, 5.0F, 0, vec.w, vec.x, vec.y, vec.z);

        drawRoundedRect(render2D.getDrawContext().pose(), x + 34.2F, y + 26F, 4 * lastAbsorption, 5.0F, 0, Color.YELLOW.getRGB());

        FontUtils.sf_bold[13].centeredDraw(render2D.getDrawContext().pose(), healthDisplay + " HP", x + 105, y + 16, -1);

        List<ItemStack> stacks = new ArrayList<>();
        stacks.add(target.getMainHandItem());
        stacks.add(target.getOffhandItem());
        stacks.removeIf(i -> i.getItem() instanceof AirItem || i.isEmpty());

        float renderOffset = 0;
        for (ItemStack stack : stacks) {
            render2D.getDrawContext().pose().pushPose();
            render2D.getDrawContext().pose().translate(x + renderOffset + 35, y + 16, 0);
            render2D.getDrawContext().pose().scale(0.5f, 0.5f, 1.0f);
            render2D.getDrawContext().renderItem(stack, 0, 0, 7, 0);
            render2D.getDrawContext().renderItemDecorations(mc.font, stack, 0, 0);
            render2D.getDrawContext().pose().popPose();
            renderOffset += 15;
        }
        render2D.getMatrixStack().popPose();
    ...
    private void waterMark(EventRender2D render2D) {
        ...
        var matrices = render2D.getDrawContext().pose();
    ...
    private void сoordinates(EventRender2D render2D) {
        ...
        if (isLeftSide) {
            font.drawLeftAligned(render2D.getDrawContext().pose(), coords, x, y + 12, Color.white.getRGB());
            font.drawLeftAligned(render2D.getDrawContext().pose(), tps, x, y, Color.white.getRGB());
            coordinateshudDrag.setWidth(textWidth);

        } else {
            font.drawRightAligned(render2D.getDrawContext().pose(), coords, x + textWidth, y + 12, Color.white.getRGB());
            font.drawRightAligned(render2D.getDrawContext().pose(), tps, x + textWidth, y, Color.white.getRGB());
            coordinateshudDrag.setWidth(textWidth);
        }
    ...
    public LivingEntity getTarget(LivingEntity nullTarget) {
        ...
        else if (Manager.FUNCTION_MANAGER.gunAimbot.getTarget() != null) {
            target = Manager.FUNCTION_MANAGER.gunAimbot.getTarget();
            tHudAnimation.setDirection(Direction.AxisDirection.POSITIVE);
        }
        else if (visibleCrosshair.get() && mc.crosshairPickEntity instanceof LivingEntity) {
    ...
    public class StaffPlayer {
        ...
        public StaffPlayer(String name, Component prefix, @Nullable java.util.UUID uuid) {
            this.name = name;
            this.prefix = prefix;
            this.uuid = uuid;
        ...
        public void updateStatus() {
            ...
            if (entry != null) {
                var loaded = mc.level.getPlayerByUUID(entry.getProfile().getId());
                entityLoaded = (loaded != null);
            }

            if (!this.isOnPlayerList) {
                this.status = Status.VANISHED;
            } else if (this.gameMode == GameType.SPECTATOR) {
