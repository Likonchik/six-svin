...
public class AltManager extends Screen implements IMinecraft {
    ...
    public void render(GuiGraphics drawContext, int mouseX, int mouseY, float delta) {
        ...
        RenderUtil.drawRoundedRect(drawContext.pose(), -1, -1, this.width + 2, this.height + 2, 4, new Color(27, 27, 27, 255).getRGB());
        if (shakeTime > 0) {
            shakeTime--;
        ...
        FontUtils.sf_bold[48].renderAnimatedGradientText(drawContext.pose(), title, titleX, titleY, ColorUtil.getColorStyle(30), ColorUtil.getColorStyle(260), time);
        ...
        RenderUtil.drawRoundedRect(drawContext.pose(), inputX, inputY, inputWidth, inputHeight, 4, ColorUtil.rgba(35, 35, 35,255));
        if (!isTyping) {
            ...
            FontUtils.durman[21].drawLeftAligned(drawContext.pose(), placeholder.toString(), inputX + 6, inputY + inputHeight / 2f - 7, nameColor);
        } else {
            StringBuilder builder = new StringBuilder(inputText);
            builder.append((System.currentTimeMillis() / 500 % 2) == 0 ? "_" : "");
            FontUtils.durman[21].drawLeftAligned(drawContext.pose(), builder.toString(), inputX + 6, inputY + inputHeight / 2f - 7, nameColor);
        }
        ...
        RenderUtil.drawRoundedRect(drawContext.pose(), listX, listY, listWidth, listHeight, 4,new Color(35, 35, 35,255).getRGB());
        ...
        for (int i = 0; i < accounts.size(); i++) {
            ...
            RenderUtil.drawRoundedRect(drawContext.pose(), entryX, y, entryWidth + 10, entryHeight, 4, ColorUtil.rgba(23, 23, 23, 255));

            int bgColor = (i == selectedAccountIndex) ? ColorUtil.rgba(50, 50, 80, 255) : ColorUtil.rgba(23, 23, 23, 255);
            RenderUtil.drawRoundedBorder(drawContext.pose(), entryX, y, entryWidth + 10, entryHeight, 4, 0.3f, bgColor);

            FontUtils.durman[21].drawLeftAligned(drawContext.pose(), accounts.get(i), entryX + 10, y + 5, ColorUtil.rgba(200, 200, 200, 255));
            FontUtils.durman[16].drawLeftAligned(drawContext.pose(), "Date " + LocalDate.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")), entryX + 10, y + 30, ColorUtil.rgba(140, 140, 140, 255));
            ...
            RenderUtil.drawRoundedRect(drawContext.pose(), selectBtnX, selectBtnY, btnWidth, btnHeight + 2, 4, selectBgColor);
            RenderUtil.drawRoundedBorder(drawContext.pose(), selectBtnX, selectBtnY, btnWidth, btnHeight + 2, 4, 1f, outlineColor);
            FontUtils.sf_medium[20].centeredDraw(drawContext.pose(), "Select", selectBtnX + btnWidth / 2f, selectBtnY + btnHeight / 2f - 6, Color.WHITE.getRGB());
            ...
            RenderUtil.drawRoundedRect(drawContext.pose(), deleteBtnX, deleteBtnY, btnWidth, btnHeight + 2, 4, deleteBgColor);
            RenderUtil.drawRoundedBorder(drawContext.pose(), deleteBtnX, deleteBtnY, btnWidth, btnHeight + 2, 4, 1f, outlineColor);
            FontUtils.sf_medium[20].centeredDraw(drawContext.pose(), "Delete", deleteBtnX + btnWidth / 2f, deleteBtnY + btnHeight / 2f - 6, Color.WHITE.getRGB());
        }
        ...
        drawContext.pose().translate(createX + buttonWidth / 2f, buttonsY + buttonHeight / 2f, 0);
        drawContext.pose().scale(createScale, createScale, 1);
        drawContext.pose().translate(-(createX + buttonWidth / 2f), -(buttonsY + buttonHeight / 2f), 0);
        RenderUtil.drawRoundedRect(drawContext.pose(), createX, buttonsY, buttonWidth, buttonHeight, 4, createBgColor);
        RenderUtil.drawRoundedBorder(drawContext.pose(), createX, buttonsY, buttonWidth, buttonHeight, 4, 1f, new Color(60, 60, 60, 180).getRGB());
        FontUtils.sf_medium[20].centeredDraw(drawContext.pose(), "Create", createX + buttonWidth / 2f, buttonsY + buttonHeight / 2f - 7, Color.WHITE.getRGB());
        ...
        drawContext.pose().pushPose();
        drawContext.pose().translate(clearX + buttonWidth / 2f, buttonsY + buttonHeight / 2f, 0);
        drawContext.pose().scale(clearScale, clearScale, 1);
        drawContext.pose().translate(-(clearX + buttonWidth / 2f), -(buttonsY + buttonHeight / 2f), 0);
        RenderUtil.drawRoundedRect(drawContext.pose(), clearX, buttonsY, buttonWidth, buttonHeight, 4, clearBgColor);
        RenderUtil.drawRoundedBorder(drawContext.pose(), clearX, buttonsY, buttonWidth, buttonHeight, 4, 1f, new Color(60, 60, 60, 180).getRGB());
        FontUtils.sf_medium[20].centeredDraw(drawContext.pose(), "Clear all", clearX + buttonWidth / 2f, buttonsY + buttonHeight / 2f - 7, Color.WHITE.getRGB());
        drawContext.pose().popPose();
        ...
        drawContext.pose().pushPose();
        drawContext.pose().translate(randomX + buttonWidth / 2f, buttonsY + buttonHeight / 2f, 0);
        drawContext.pose().scale(randomScale, randomScale, 1);
        drawContext.pose().translate(-(randomX + buttonWidth / 2f), -(buttonsY + buttonHeight / 2f), 0);
        RenderUtil.drawRoundedRect(drawContext.pose(), randomX, buttonsY, buttonWidth, buttonHeight, 4, randomBgColor);
        RenderUtil.drawRoundedBorder(drawContext.pose(), randomX, buttonsY, buttonWidth, buttonHeight, 4, 1f, new Color(60, 60, 60, 180).getRGB());
        FontUtils.sf_medium[20].centeredDraw(drawContext.pose(), "Random", randomX + buttonWidth / 2f, buttonsY + buttonHeight / 2f - 7, Color.WHITE.getRGB());
        drawContext.pose().popPose();

        String accountName = mc.getUser().getName();
        FontUtils.sf_medium[18].centeredDraw(drawContext.pose(), "Selected account: " + accountName, centerX, buttonsY + buttonHeight + (int)(20 * SCALE), -1);
        FontUtils.sf_medium[18].centeredDraw(drawContext.pose(), "Quantity: " + accounts.size(), centerX, buttonsY + buttonHeight + (int)(40 * SCALE), -1);
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (isTyping) {
            boolean ctrl = GLFW.glfwGetKey(mc.getWindow().getWindow(), GLFW.GLFW_KEY_LEFT_CONTROL) == GLFW.GLFW_PRESS || GLFW.glfwGetKey(mc.getWindow().getWindow(), GLFW.GLFW_KEY_RIGHT_CONTROL) == GLFW.GLFW_PRESS;
    ...
    private void drawConfirmDialog(GuiGraphics drawContext) {
        int boxWidth = 300;
        int boxHeight = 130;
        int boxX = (width - boxWidth) / 2;
        int boxY = (height - boxHeight) / 2;

        RenderUtil.drawRoundedRect(drawContext.pose(), 0, 0, width, height, 0, new Color(0, 0, 0, 120).getRGB());

        RenderUtil.drawRoundedRect(drawContext.pose(), boxX, boxY, boxWidth, boxHeight, 6, new Color(40, 40, 40, 240).getRGB());

        FontUtils.sf_bold[22].centeredDraw(drawContext.pose(), "Вы точно хотите очистить все аккаунты?", width / 2f, boxY + 30, Color.WHITE.getRGB());
        ...
        RenderUtil.drawRoundedRect(drawContext.pose(), yesX, btnY, btnWidth, btnHeight, 5, new Color(60, 180, 75).getRGB());
        FontUtils.sf_medium[20].centeredDraw(drawContext.pose(), "Да", yesX + btnWidth / 2f, btnY + btnHeight / 2f - 6, Color.WHITE.getRGB());

        RenderUtil.drawRoundedRect(drawContext.pose(), noX, btnY, btnWidth, btnHeight, 5, new Color(200, 60, 60).getRGB());
        FontUtils.sf_medium[20].centeredDraw(drawContext.pose(), "Нет", noX + btnWidth / 2f, btnY + btnHeight / 2f - 6, Color.WHITE.getRGB());
    }
}
