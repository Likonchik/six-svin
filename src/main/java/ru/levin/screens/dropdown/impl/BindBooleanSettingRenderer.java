...
public class BindBooleanSettingRenderer implements SettingRenderer<BindBooleanSetting>, IMinecraft {
    ...
    @Override
    public void render(GuiGraphics ctx, BindBooleanSetting setting, int x, int y, int width, int height) {
        ...
        if (setting.expanded) {
            ...
            RenderUtil.drawRoundedRect(ctx.pose(), bindBoxX, bindBoxY, bindBoxWidth, bindBoxHeight, 3, new Color(40, 40, 40, 200).getRGB());
            ...
            FontUtils.durman[13].drawLeftAligned(ctx.pose(), displayText, bindBoxX + 5 - offset, bindBoxY + (bindBoxHeight - FontUtils.durman[13].getHeight()) / 2f, Color.WHITE.getRGB());

            int closeSize = 12;
            int paddingRightClose = 8;
            int closeX = x + width - closeSize - paddingRightClose + 10;
            int closeY = bindBoxY + (bindBoxHeight - closeSize) / 2 - 1;
            RenderUtil.drawTexture(ctx.pose(), "images/gui/fl.png", closeX, closeY, closeSize, closeSize, 0, Color.RED.getRGB());
        ...
        RenderUtil.drawRoundedRect(ctx.pose(), switchX, switchY, SWITCH_WIDTH, SWITCH_HEIGHT, 5, bgColor);
        int knobRadius = 8;
        float knobX = switchX + 3 + (SWITCH_WIDTH - knobRadius - 5) * progress;
        float knobY = switchY + (SWITCH_HEIGHT - knobRadius) / 2f;
        RenderUtil.drawCircle(ctx.pose(), knobX + knobRadius / 2f, knobY + knobRadius / 2f, knobRadius, Color.WHITE.getRGB());
