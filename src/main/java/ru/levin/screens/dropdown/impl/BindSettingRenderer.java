...
public class BindSettingRenderer implements SettingRenderer<BindSetting>, IMinecraft {
    ...
    @Override
    public void render(GuiGraphics ctx, BindSetting setting, int x, int y, int width, int height) {
        String displayText;
        ...
        FontUtils.durman[fontSizeButton].centeredDraw(ctx.pose(), displayText, buttonX + buttonWidth / 2f, buttonY + (buttonHeight - FontUtils.durman[fontSizeButton].getHeight()) / 2f, Color.WHITE.getRGB());
    }
