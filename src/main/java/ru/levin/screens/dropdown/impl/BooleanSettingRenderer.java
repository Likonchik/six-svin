...
public class BooleanSettingRenderer implements SettingRenderer<BooleanSetting>, IMinecraft {
    ...
    @Override
    public void render(GuiGraphics ctx, BooleanSetting setting, int x, int y, int width, int height) {
        ...
        RenderUtil.drawRoundedRect(ctx.pose(), switchX, switchY, WIDTH, SWITCH_HEIGHT, 5, bgColor);
        RenderUtil.drawCircle(ctx.pose(), switchX + 3 + (WIDTH - KNOB_RADIUS - 5) * progress + KNOB_RADIUS / 2f, switchY + (SWITCH_HEIGHT - KNOB_RADIUS) / 2f + KNOB_RADIUS / 2f, KNOB_RADIUS, Color.WHITE.getRGB());
