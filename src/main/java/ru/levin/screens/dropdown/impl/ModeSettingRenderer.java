...
public class ModeSettingRenderer implements SettingRenderer<ModeSetting>, IMinecraft {
    ...
    @Override
    public void render(GuiGraphics ctx, ModeSetting setting, int x, int y, int width, int height) {
        ...
        for (String mode : setting.getModes()) {
            ...
            RenderUtil.drawRoundedRect(ctx.pose(), currentX, currentY, boxWidth, BOX_HEIGHT, 1, bgColor);
            ...
            FontUtils.durman[13].drawLeftAligned(ctx.pose(), mode, currentX + PADDING_HORIZONTAL - offset, currentY + (BOX_HEIGHT - FontUtils.durman[13].getHeight()) / 2f, textColor);
            Scissor.pop();
