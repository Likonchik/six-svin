...
public class SliderSettingRenderer implements SettingRenderer<SliderSetting>, IMinecraft {
    ...
    @Override
    public void render(GuiGraphics ctx, SliderSetting setting, int x, int y, int width, int height) {
        int barWidth = width - 2 * PADDING;
        int barX = x + PADDING;
        int barY = y + height - BAR_HEIGHT - 4;

        RenderUtil.drawRoundedRect(ctx.pose(), barX, barY, barWidth, BAR_HEIGHT, 1, new Color(50, 50, 50, 180).getRGB());
