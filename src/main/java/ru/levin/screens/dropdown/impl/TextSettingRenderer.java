...
public class TextSettingRenderer implements SettingRenderer<TextSetting> {
    ...
    @Override
    public void render(GuiGraphics ctx, TextSetting setting, int x, int y, int width, int height) {
        int fontSizeName = 13;
        ...
        RenderUtil.drawRoundedRect(ctx.pose(), fieldX, fieldY, fieldWidth, fieldHeight - 1, FIELD_RADIUS, bgColor);
        ...
        FontUtils.durman[fontSizeField].drawLeftAligned(ctx.pose(), value, fieldX + HORIZONTAL_PADDING + scrollOffset, fieldY + VERTICAL_PADDING - 0.6f, Color.WHITE.getRGB());
        Scissor.pop();
        ...
        if (setting.isFocused()) {
            ...
            if (setting.cursorVisible) {
                ...
                RenderUtil.drawRoundedRect(ctx.pose(), fieldX + HORIZONTAL_PADDING + cursorPos + scrollOffset, fieldY + VERTICAL_PADDING, 1, FontUtils.durman[fontSizeField].getHeight(), 0, Color.WHITE.getRGB());
            }
        }
    }
