...
public class MathUtil implements IMinecraft {
    ...
    public static float fast(float end, float start, float multiple) {
        return (1 - Mth.clamp((float) (deltaTick() * multiple), 0, 1)) * end + Mth.clamp((float) (deltaTick() * multiple), 0, 1) * start;
    }
    public static float lerp(float end, float start, float multiple) {
        return (float) (end + (start - end) * Mth.clamp(deltaTick() * multiple, 0, 1));
    }
    public static double lerp(double end, double start, double multiple) {
        return (end + (start - end) * Mth.clamp(deltaTick() * multiple, 0, 1));
    }
    public static float faster(float current, float target, float speed) {
        float diff = target - current;
        float deltaTime = 1F / mc.getFps();
    ...
    public static float interpolateAngle(float start, float end, float progress) {
        ...
        return start + difference * Mth.clamp(progress, 0, 1);
    }
    ...
    public static Vec3 getClosestVec(Vec3 vec, AABB AABB) {
        return new Vec3(clamp(vec.x, AABB.minX, AABB.maxX), clamp(vec.y, AABB.minY, AABB.maxY), clamp(vec.z, AABB.minZ, AABB.maxZ));
    }
    ...
    public static float interpolateSmooth(double smooth, float prev, float orig) {
        return (float) lerp(mc.getTimer().getRealtimeDeltaTicks() / smooth, prev, orig);
    }
