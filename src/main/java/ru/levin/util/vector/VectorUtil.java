...
public class VectorUtil implements IMinecraft {
    ...
    public static PoseStack.Pose lastWorldSpaceMatrix = new PoseStack().last();
    ...
    public static Vector3d toScreen(double x, double y, double z) {
        ...
        Vector4f vector4f = new Vector4f(vector3f.x, vector3f.y, vector3f.z, 1.0f).mul(lastWorldSpaceMatrix.pose());
    ...
    public static Vector2f project(double x, double y, double z) {
        ...
        Quaternionf rotation = new Quaternionf(mc.getEntityRenderDispatcher().cameraOrientation()).conjugate();
        pos.rotate(rotation);
        ...
        double fov = ((GameRendererAccessor) mc.gameRenderer).invokeGetFov(camera, mc.getTimer().getGameTimeDeltaPartialTick(true), true);
        return calculateScreenPosition(pos, fov);
    }

    private static void applyViewBobbing(Player player, Vector3f pos) {
        float delta = mc.getTimer().getGameTimeDeltaPartialTick(true);
        float speed = Mth.lerp(delta, player.walkDistO, player.walkDist);
        float bob = player.yya - player.xxa;

        if (bob != 0) {
            float roll = Mth.sin((float) (bob * Math.PI)) * speed * 3f;
            pos.rotateZ(roll * ((float) Math.PI / 180f));

            float pitch = Math.abs(Mth.cos((float) (bob * Math.PI - 0.2f)) * speed) * 5f;
            pos.rotateX(pitch * ((float) Math.PI / 180f));

        }

        pos.add(Mth.sin((float) (bob * Math.PI)) * speed * 0.5f, -Math.abs(Mth.cos((float) (bob * Math.PI)) * speed), 0f);
    }
    ...
    private static Vector2f calculateScreenPosition(Vector3f pos, double fov) {
        ...
        float halfW = w.getGuiScaledWidth() * 0.5f;
        float halfH = w.getGuiScaledHeight() * 0.5f;
